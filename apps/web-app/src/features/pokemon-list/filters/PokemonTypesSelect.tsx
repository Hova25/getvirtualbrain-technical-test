...
export const PokemonTypesSelect = () => {
  ...
  const handleChange = (newValue: MultiValue<PokemonType>) => {
    if (newValue.length > 0) {
      searchParams.set(
        POKEMON_LIST_SEARCH_PARAM_TYPES,
        newValue.map(({ name }) => name).toString(),
      );
  ...
  return (
    <Select
      ...
      name="pokemonTypes"
      className="w-full border-amber-300 focus:ring-2 focus:ring-amber-600 sm:w-96 dark:!text-gray-200"
      classNames={{
        control: ({ isFocused }) =>
        ...
        option: ({ isFocused, isSelected }) =>
          `transition-colors ${isFocused ? "!bg-amber-300 dark:!text-gray-800" : "!text-black !bg-gray-200 dark:!text-gray-200 dark:!bg-gray-800"} ${isSelected ? "!bg-amber-200" : ""}`,
      }}
